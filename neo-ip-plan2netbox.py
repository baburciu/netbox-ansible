...
assigned_device = sh.col_values(0, start_rowx=1)
...
for i in range(len(ip_addr)):
    if "nope" not in prefix[i]:
        ...
        r = ansible_runner.run(private_data_dir='/home/ubuntu/netbox-ansible/',
                               ...
                               extravars={'prefix': prefix[i], 'site_name': "Lannion-TGI-Labs",
    ...
    if "vlan" in assigned_interface[i]:
        ...
        print(f"******* Now we'll create NetBox interface {assigned_interface[i]} for device {assigned_device[i]}")
        r = ansible_runner.run(private_data_dir='/home/ubuntu/netbox-ansible/',
                               playbook='create_interface.yml',
                               inventory='/home/ubuntu/netbox-ansible/hosts.yml',
                               extravars={'interface_device': assigned_device[i], 'interface_name': assigned_interface[i],
                                          'interface_mac_address': assigned_device_vlans_mac[i],
    ...
    if "nope" not in assigned_role[i]:
        # NetBox: create the IP address (with a role) and assign it to an interface
        print(f"******* Now we'll create NetBox IP address {ip_addr[i]} for device {assigned_device[i]}'s interface {assigned_interface[i]}")
        r = ansible_runner.run(private_data_dir='/home/ubuntu/netbox-ansible/',
                               ...
                               extravars={'ip_addr_address': ip_addr[i], 'ip_addr_tenant': assigned_tenant[i],
                                          ...
                                          'ip_addr_interface_name': assigned_interface[i], 'ip_addr_interface_device': assigned_device[i],
                                          'ip_addr_role': assigned_role[i],
                                          'external_vars': './external_vars.yml',
                                          'ansible_python_interpreter': '/usr/bin/python3'})
    else:
        # NetBox: create the IP address (without a role) and assign it to an interface
        print(f"******* Now we'll create NetBox IP address {ip_addr[i]} for device {assigned_device[i]}'s interface {assigned_interface[i]}")
        r = ansible_runner.run(private_data_dir='/home/ubuntu/netbox-ansible/',
                               playbook='create_ip_addr.yml',
                               inventory='/home/ubuntu/netbox-ansible/hosts.yml',
